...
class HomographyGridGen(nn.Module):
    def __init__(self, out_h=256, out_w=192, out_ch=3):
        ...
        self.points = torch.from_numpy(stack[:,:,None,None,:]).cuda().float()

        self._3x3_ = nn.Linear(50,9)
        
    def forward(self, theta):
        batch_size = theta.size()[0]
        theta = self._3x3_(theta.contiguous().view(batch_size,-1)).view(batch_size,3,3)

        grid = self.points @ theta

        grid = grid[:,:,:,0,:2].view(-1,batch_size,2).transpose(0,1)
        grid = grid.view(batch_size, self.out_h, self.out_w, 2)
        return grid
...
class GMM(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #self.gridGen = HomographyGridGen(opt.fine_height, opt.fine_width, opt.grid_size)
